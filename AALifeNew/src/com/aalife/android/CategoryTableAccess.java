...
public class CategoryTableAccess {
	...
	public List<Map<String, String>> findAllSyncCat() {
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		String sql = "SELECT CategoryID, CategoryName, CategoryDisplay, CategoryLive FROM " + CATTABNAME + " WHERE Synchronize='1'";
		try {
			Cursor result = this.db.rawQuery(sql, null);
			while (result.moveToNext()) {
				Map<String, String> map = new HashMap<String, String>();
	...
	//保存网络类别
	public boolean saveWebCategory(int catId, String catName, int catDisplay, int catLive) {
		String sql = "SELECT * FROM " + CATTABNAME + " WHERE CategoryID='" + catId + "' AND CategoryLive='1'";
		try {
			Cursor result = this.db.rawQuery(sql, null);
			if (result.moveToNext()) {
	...
	//删除类别
	public boolean delCategory(int catId) {
	...
	public boolean updateCatDisplay(int catId, int display) {
		String sql = "UPDATE " + CATTABNAME + " SET CategoryDisplay='" + display + "', Synchronize='1' WHERE CategoryID=" + catId;
		try {
		    this.db.execSQL(sql);
		} catch (Exception e) {
			e.printStackTrace();
